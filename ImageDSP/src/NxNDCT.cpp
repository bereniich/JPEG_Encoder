...
void GenerateDCTmatrix(double* DCTKernel, int order) {
    int N = order;
    double alpha0 = sqrt(1.0 / (double)N);
    double alphaN = sqrt(2.0 / (double)N);

    for (int j = 0; j < N; j++) {
        DCTKernel[0 * N + j] = alpha0;
    }

    for (int i = 1; i < N; i++) {
        for (int j = 0; j < N; j++) {
            DCTKernel[i * N + j] = alphaN * cos(((2 * j + 1) * i * M_PI) / (2.0 * N));
        }
    }
}
...
void DCT(const char input[], int16_t output[], int N, double* DCTKernel)
{
    ...
    for (int i = 0; i <= N - 1; i++)
    {
        for (int j = 0; j <= N - 1; j++)
        {
            ...
            for (int k = 0; k <= N - 1; k++)
            {
                sum = sum + DCTKernel[i*N+k] * ((double)(unsigned char)input[k*N+j] - 128.0);            }
            temp[i*N + j] = sum;
        }
    }
